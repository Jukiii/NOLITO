...
| 最終更新日時   | 2026-09-24                                      |
| 現在のフェーズ | Phase 21(テーマ・アクセシビリティ強化)。PR2完了 |
| 現在の状態     | IN_PROGRESS                                     |
...
| Phase 21 | テーマ(ライト/ダーク)・アクセシビリティ強化                              | IN_PROGRESS   | PR1=#67, PR2=#69                                              | -                                              | PR1(テーマ)・PR2(文字サイズ・アニメーション軽減・キーボード/コントラスト点検)完了・マージ・本番確認済み(0046決定ログ)。PR3は未着手 |
...
```text
Phase 21(PR 1・PR 2 完了。PR 3: テーマ・文字サイズ・アニメーション軽減のアカウント同期、は未着手)
```

## Current Status

```text
IN_PROGRESS(Phase 21 PR 2 は完了。次は PR 3)
```

## Current Branch

```text
main(PR 2 は #69 でマージ済み。PR 3 用のブランチは、まだ作成していない)
```

## Current Task

```text
Phase 21 PR 3(テーマ・文字サイズ・アニメーション軽減の設定を、アカウントに同期する)の設計。
Phase 19 PR2(game_progress の同期)と同じ考え方(要約だけを送る・任意・アップロードは即時、
ダウンロードは確認ダイアログ経由)を踏襲する予定。着手時に、詳細決定を 0046 決定ログに追記する。
```

## Current Step

```text
Phase 21 PR 3 の仕様確認・実装計画の提示
```
...
```text
Phase 21 PR 2(文字サイズ・アニメーション軽減の切替、キーボード/コントラスト点検)を実装・テスト・
マージ・本番確認まで完了した。次は PR 3(テーマ・文字サイズ・アニメーション軽減のアカウント同期)。
```

### 次に行う作業

```text
1. Phase 21 PR 3 の仕様確認と、詳細決定を 0046 決定ログに追記
2. phase-21-settings-sync(仮)ブランチ(main から)で、PR 3 に着手する
```
...
マージ済み、本番 nolito.pages.dev で確認済み)。components/font-size.js・components/motion.js(新規)。
文字サイズは標準/大きめ/特大(ルートのfont-sizeを変えるだけで、rem連動でサイト全体が拡大)。
アニメーション軽減は「システムに合わせる」「減らす」の2つだけ(強制的に動きを戻す選択肢はない)。
ゲームのmain.jsは、prefersReducedMotion()経由でOS+サイト設定を判定。FOUC防止スクリプトに両方の
読み取りを追加。サイト全体キーボード操作の点検(headless Edgeで計12件)・コントラスト/色以外の
表現の点検(コードレビュー)を実施し、どちらも既存実装が基準を満たすことを確認(コード修正なし)。
単体テスト1868件・実ブラウザE2E計22件、すべて成功。決定・点検結果は docs/decisions/0046-phase-21-plan.md。

Phase 21 PR 1: ライト・ダーク・システムテーマ(PR #67、マージ済み、本番確認済み)。
```
...
```text
(PR #69 のマージコミット。git log --oneline -1 で確認)
```

## Pull Request

```text
直近マージ: #69(Phase 21 PR 2)、#68(chore)、#67(Phase 21 PR 1)、#66(chore)、#65(Phase 20 PR 3)、
#64(Phase 20 PR 2)、#63(Phase 20 PR 1)
オープン中: なし
```
...
Phase:
Phase 21(ライト・ダーク・システムテーマ。サイト全体キーボード操作。アクセシビリティ強化)。PR1・PR2完了

Status:
IN_PROGRESS

Branch:
main(Phase 21 PR1・PR2 がマージ済み。PR3 用ブランチは、まだ未作成)

Task:
Phase 21 PR3(テーマ・文字サイズ・アニメーション軽減の設定を、アカウントに同期する)の仕様確認・実装計画

Last Completed:
**Phase 21 PR2(文字サイズ・アニメーション軽減の切替、キーボード/コントラスト点検)を、完了した**
(PR #69、マージ済み)。`components/font-size.js`・`components/motion.js`(新規。theme.js と同じ形の
保存・読込・反映)。文字サイズは標準/大きめ/特大(`localStorage` の `nolito:font-size:v1`。ルートの
font-sizeだけを変え、`--font-size-*` はremのままなので、サイト全体が連動して拡大)。アニメーション
軽減は「システムに合わせる」「減らす」の2つだけ(`nolito:motion:v1`。OSが減らす指定のとき、サイト側で
動きを強制的に戻す選択肢は作らない、という意図的な設計)。ゲームの`main.js`は、`prefersReducedMotion()`
(OS+サイト設定の両方を判定)経由に変更。FOUC防止スクリプト(全ページ)に、両方の読み取りを追加。
**サイト全体キーボード操作の点検**(headless Edgeで、ヘッダードロップダウン・検索フォーム・フッター
設定・ゲームダッシュボード・モバイルメニューを計12件確認)と、**コントラスト/色以外の表現の点検**
(バッジ・エラー表示・ゲームの危険状態表示をコードレビュー)を実施し、どちらも既存実装が基準を
満たすことを確認した(この点検を理由にしたコード修正はなし)。単体テスト1868件・実ブラウザE2E計22件、
すべて成功。本番(nolito.pages.dev)で確認済み。静的サイトのみの変更のため、本番マイグレーションは
不要。決定・点検結果・テスト結果は docs/decisions/0046-phase-21-plan.md。CLAUDE.md に「サイト内文字
サイズ・アニメーション軽減の切替(Phase 21 PR 2)」節を追記。

Phase 21 PR1(ライト・ダーク・システムテーマ。PR #67)も、この前に完了・マージ・本番確認済み。

Next Action:
Phase 21 PR3(アカウント同期)の詳細を、0046 決定ログに追記してから着手する。Phase 19 PR2
(`game_progress`の同期)と同じ考え方(要約だけを送る・任意・アップロードは即時、ダウンロードは
確認ダイアログ経由・D1に新しいテーブル/マイグレーションが必要)を踏襲する想定。
