...
| 最終更新日時   | 2026-09-24                                                       |
| 現在のフェーズ | Phase 20(拡張可能なヘッダーメニュー・トップページ・検索)。未着手 |
| 現在の状態     | IN_PROGRESS                                                      |
...
Phase:
Phase 20(拡張可能なヘッダーメニュー・トップページ・検索)。未着手

Status:
IN_PROGRESS

Branch:
main(Phase 19 の全 PR がマージ済み。Phase 20 用ブランチは、まだ未作成)

Task:
Phase 20(docs/01_phases/phase-20.md)の仕様確認・実装計画

Last Completed:
**Phase 19(未登録プレイ+アカウント移行・公開範囲設定・オンラインランキング)を、完了した**。
PR1(#56。記録のJSON書き出し・読み込み)・PR2(#58。ゲームの記録のアカウント移行=同期)・
PR3(#61。プロフィール公開範囲の設定・オンラインランキング)を、すべてマージ。
migrations/0004_game_sync.sql・0005_ranking.sql は、どちらも本番の D1 に適用済み(運営者の許可を
得て実施。本番の API(/api/games/escape-boss/sync・/api/games/escape-boss/ranking)が、テーブル
なしのエラーではなく、正しい応答を返すことで確認した)。決定は
docs/decisions/0042〜0044-phase-19-*.md。設定画面からのアカウント削除は、Phase 9 で実装済み。

Next Action:
Phase 20 の仕様書(docs/01_phases/phase-20.md)を読み、実装計画を立てる。範囲が広い
(ヘッダーメニュー拡張・トップページ・カテゴリ追加・カスタムページ・検索)ため、Phase 18・19 と
同じく、複数の PR に分ける計画を、決定ログ(docs/decisions/)に残してから着手する。
