...
| 最終更新日時   | 2026-09-24                                    |
| 現在のフェーズ | Phase 19(PR 1 完了。PR 2 は運営者の確認待ち） |
| 現在の状態     | IN_PROGRESS                                   |
...
| Phase 19 | 未登録プレイ+アカウント移行・公開範囲設定・オンラインランキング          | **IN_PROGRESS** | PR1=#56(済), PR2以降=未着手        | -                                              | PR2・3は、着手前にプライバシーポリシー改定を運営者に確認(0042決定ログ） |
...
```text
Phase 19(PR 1 完了。PR 2: アカウント移行、に着手前)
```
...
```text
main(PR 1 マージ済み。PR 2 用のブランチは、まだ作成していない)
```

## Current Task

```text
Phase 19 PR 2(アカウント移行・初回アクセス時の登録案内)の計画。
着手前に、プライバシーポリシーの改定内容(記録をサーバーに送ることの開示)を、運営者に確認する。
```
...
Phase:
Phase 19 PR 2(アカウント移行・初回アクセス時の登録案内)

Status:
IN_PROGRESS(PR1 完了。PR2 は着手前の確認待ち)

Branch:
main(PR2 用ブランチは、まだ未作成)

Task:
PR2(アカウント移行)着手前に、プライバシーポリシーの改定内容(ゲームの記録を、初めて運営者の
サーバーに送ることの開示)を、運営者に確認する

Last Completed:
Phase 19 PR 1(記録のJSON書き出し・読み込み)。PR #56 マージ・本番 0.24.0 で確認済み

Next Action:
プライバシーポリシーの改定案(6-2アカウントの節に、ゲームの記録の保存を追記。版を上げて同意を
取り直す)を作り、運営者に確認してもらう。確認が取れたら、phase-19-account-sync ブランチで、
D1のスキーマ設計・同期API・初回アクセス時の登録案内バナーを実装する。

Human Task:
Phase 19 PR2 着手前の、プライバシーポリシー改定内容の確認(上記)。
ほかに、Issue #12・#13・#19 は WAITING_HUMAN だが、いまの作業のブロッカーではない
