...
| 最終更新日時   | 2026-09-24                                 |
| 現在のフェーズ | Phase 19(PR 1・PR 2 完了。PR 3 は未着手）  |
| 現在の状態     | IN_PROGRESS                                |
...
| Phase 19 | 未登録プレイ+アカウント移行・公開範囲設定・オンラインランキング          | **IN_PROGRESS** | PR1=#56(済), PR2=#58(済・本番マイグレーション適用済み), PR3=未着手 | -                                              | PR3(プロフィール公開範囲・オンラインランキング)の設計が次(0043決定ログ） |
...
```text
IN_PROGRESS(Phase 19 PR 2 は完了。次は PR 3)
```
...
```text
Phase 19 PR 3(プロフィール公開範囲の設定・オンラインランキング)の設計。0042 決定ログのとおり、
招待制のままでよい機能。オンラインランキング用のクエリに、いまの game_progress(1アカウント1行の
要約)で足りるか、別の構造が要るかを、PR3 着手時に検討する(0043 決定ログの既知事項)。
```

## Current Step

```text
Phase 19 PR 3 の仕様確認・実装計画の提示
```
...
Task:
Phase 19 PR 3(プロフィール公開範囲の設定・オンラインランキング)の仕様確認・実装計画

Last Completed:
Phase 19 PR 2(ゲームの記録のアカウント移行・同期)。PR #58 マージ済み(コード・テスト・
プライバシーポリシー改定(版3)・ローカルでの実ブラウザ E2E(18件)まで完了)。
migrations/0004_game_sync.sql は、本番の D1 に適用済み(運営者の許可を得て実施。num_tables が
6 から 7 に増え、本番の /api/games/escape-boss/sync が、テーブルなしのエラーではなく
{"error":"not-logged-in"} を返すことで確認した)。決定は docs/decisions/0043-phase-19-account-sync.md

Next Action:
Phase 19 PR 3(プロフィール公開範囲の設定・オンラインランキング)に着手する。招待制のままでよい
(0042決定ログ)。いまの game_progress(1アカウント1行の要約)で、オンラインランキングの
「上位N件」のようなクエリに足りるか、別の構造が要るかを、着手時に検討する(0043の既知事項)

Human Task:
なし(このフェーズの直接のブロッカーではない)。Issue #12・#13・#19 は WAITING_HUMAN のまま残っている
