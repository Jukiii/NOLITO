...
- 語録は職種ごと30語(計180語。決定ログ 0021)+ 任意の拡張ファイル(役職・難易度の専用語。Phase 24・0049)。**語の id は、消さない・つけ替えない**(記録・復習リストが id で語を引く)。難易度は**読みの長さの決め**(単位数 ≤3 → 1、4〜5 → 2、≥6 → 3。小さい ゃゅょぁぃぅぇぉ は前の字と合わせて 1)で、`npm run vocab:stats` の警告が 0 であることを、テストが検査する。語を足す・変えたら `npm run vocab:review` で確認シートを作り直し、語の長さが変わって、クリア率が動きそうなときは、シミュレーションで `roles.json` を見直す(0006・0021)。
...
## 語録の拡張ファイル・役職/難易度専用語(Phase 24)

Phase 24 の仕様(「難易度・役職拡張ファイル」)は解釈が複数あったため、Issue #85 で運営者に確認し、**役職・難易度ごとの専用語**を実装する方針で承認を得た(決定は `docs/decisions/0049-phase-24-plan.md`)。**0021(役職ごとの出し分けはしない)は、この専用語というしくみで、個別の語ごとに上書きできるようになった**。

- **役職専用語**: 語の `roles`(出題する役職 id の一覧。もともと必須項目)に、一部の役職だけを書けば、その役職限定になる(`vocabulary.js` の `pickWords` が、既存のフィルタでそのまま対応)。既存の 180 語は、変更していない(引き続き全役職が対象)。
...
### 類似語チェック(Phase 25 PR 2)

- **完全一致(id・日本語表記・読み)は、既存の `validateVocabulary`・`vocab-stats.mjs` の `findIssues` が検出済み**。新しく作ったのは、**表記は違うが、説明の文章が近い(紛らわしい・実質同じかもしれない)組**を見つける、`scripts/lib/vocab-similarity.mjs`(DOM・ファイルに触れない純粋関数)。`similarPairs(items, { threshold = 0.6 })` が、説明(`explanation`)の**文字2-gramのDice係数**(有料APIを使わない、決まった計算だけの目安)で、しきい値以上の組を、スコアの高い順に返す。職種をまたいで比較する(概念の重複は、職種の境界と関係ないため)。
- `vocab-check.mjs` の警告(プレーン表示・`--for-ai` の機械の確認結果の、両方)に、この候補を追加した。**`--improve`(改善提案)には含めない**(別の関心ごとのため)。
- **しきい値以上でも、必ず重複とは限らない**(目安。人間・AIが、最終確認する)。実データでは、しきい値0.6で2組: 1件は実際に意味が近い指摘(「仕込み」と「下ごしらえ」)、もう1件は、説明文の言い回しが偶然近いだけの誤検出(「顧客」と「納期」)。この2例が、この機能の「目安」という位置づけを、そのまま示している。

### AI活用の流れの文書化(Phase 25 PR 3)

- `docs/06_ai/README.md`(新規)に、**生成 → 機械チェック(形式・重複・類似語)→ AIチェック → 人間の最終確認 → 確認済みにする → 公開 →(公開後・任意)改善提案**、という全体の流れを、1つの図・表にまとめた。既存の各プロンプト文書・`docs/05_checklists/vocabulary-validation.md`・`docs/04_templates/vocabulary-template.md`は、内容を変えず、この文書から参照する。
- **「有料APIとの自動連携は、作らない」「管理画面からのアップロードは、この文書の範囲外(Phase 26 待ち)」を、明記**。AIを使わずに(1・3・7=生成・AIチェック・改善提案を飛ばして)運用できることも、明記。
- **Phase 25(公開済みの語への改善提案 → 類似語チェック → 流れの文書化)は、この PR で完了**。

## 管理画面・RBAC(Phase 26)

対象(語録・記事・ゲーム設定・プロダクト・ユーザー・ランキング・問い合わせ・更新履歴)が多く、この案件で最も大きいフェーズの1つ。着手前に、チャットで運営者に、最初のPRの範囲(基盤=管理者ゲート+監査ログだけ)と、RBACの粒度(単一の「管理者」フラグ)を確認した(決定は `docs/decisions/0051-phase-26-plan.md`)。

### 管理者ゲート・監査ログの基盤(Phase 26 PR 1)

- **管理者の判定は、環境変数 `ADMIN_EMAILS`**(カンマ区切り。`functions/_lib/config.js` の `adminEmails`・`isAdmin`)。既存の招待制(`ALLOWED_EMAILS`・`isInvited`)と、まったく同じパターン。**DB にフラグを持たせない**(環境変数の変更だけで、即座に有効/無効にできる)。**いまは、単一の「管理者」フラグだけ**(役職ごとの細かい権限分けは、複数の管理者が必要になってから)。
- `functions/_lib/guard.js` の **`requireAdmin(context, options)`**(`requireUser` を呼んだあと、管理者を確認。管理者でなければ `403 not-admin`)を、将来の管理画面のAPIが、共通の入り口として使う。`/api/me` の応答にも `isAdmin`(真偽値)を追加した。
