...
| 最終更新日時   | 2026-09-25                                           |
| 現在のフェーズ | Phase 22(キーボード/タッチ操作・端末最適化)。PR1完了 |
| 現在の状態     | IN_PROGRESS                                          |
...
| Phase 22 | キーボード/タッチ操作・端末最適化                                        | IN_PROGRESS   | PR1=#73                                                       | -                                              | PR1(モバイル・ソフトウェアキーボード対応・縦横自動調整)完了・マージ・本番確認済み(0047決定ログ)。PR2・PR3は未着手 |
...
```text
Phase 22(キーボード/タッチ操作・端末最適化)。PR1完了。次はPR2(タッチ操作の点検・改善)
```

## Current Status

```text
IN_PROGRESS(Phase 22 PR1 完了。次はPR2)
```

## Current Branch

```text
main(Phase 22 PR1 は #73 でマージ済み。PR2 用のブランチは、まだ作成していない)
```

## Current Task

```text
Phase 22 PR2(タッチ操作の点検・改善)の仕様確認・実装計画。0047決定ログのPR分割のとおり。
```

## Current Step

```text
Phase 22 PR2 の詳細を、0047 決定ログに追記してから着手する
```
...
```text
Phase 22 PR1(モバイル・ソフトウェアキーボード対応・縦横自動調整・回転対応)を実装・テスト・
マージ・本番確認まで完了した。次は PR2(タッチ操作の点検・改善)。
```

### 次に行う作業

```text
1. Phase 22 PR2(タッチ操作の点検・改善)の詳細を、0047 決定ログに追記
2. phase-22-touch(仮)ブランチ(main から)で、PR2 に着手する
```

### 最後に完了した作業

```text
Phase 22 PR1: モバイル・ソフトウェアキーボード対応・縦横自動調整・回転対応(PR #73、マージ済み、
本番確認済み)。プレイ中は下部固定バーを隠す(document.body[data-hide-bottom-nav])。ソフトウェア
キーボードが開いたとき(visualViewportのresize)だけ、入力欄へスクロール(inputのfocusイベント
自体では動かさない)。ゲームのinput.focus()すべてにpreventScroll:trueを追加(実ブラウザE2Eで
見つけた、デスクトップでの不要なスクロールを修正)。横向きの低い画面向けCSS(game.cssの末尾。
ヘッダー高さ・余白・文字サイズを圧縮。並び順は変えない)。kii-michiの「キーを押して入力」は、
物理キーボード前提の既知の制限として対応しないことを明記。単体テスト1890件・実ブラウザE2E9件、
すべて成功。決定・テスト結果・実装中に見つけた不具合の記録は docs/decisions/0047-phase-22-plan.md。

Phase 21(PR1=#67・PR2=#69・PR3=#71)は、その前にすべて完了・マージ・本番確認済み。
```
...
```text
(PR #73 のマージコミット。git log --oneline -1 で確認)
```

## Pull Request

```text
直近マージ: #73(Phase 22 PR 1)、#72(chore)、#71(Phase 21 PR 3。Phase 21 完了)、
#69(Phase 21 PR 2)、#68(chore)、#67(Phase 21 PR 1)
オープン中: なし
```
...
Phase:
Phase 22(キーボード/タッチ操作・端末最適化)。PR1完了(**Phase 21 は 3 PR すべて完了**)

Status:
IN_PROGRESS

Branch:
main(Phase 22 PR1 がマージ済み。PR2 用ブランチは、まだ未作成)

Task:
Phase 22 PR2(タッチ操作の点検・改善)の仕様確認・実装計画

Last Completed:
**Phase 22 PR1(モバイル・ソフトウェアキーボード対応・縦横自動調整・回転対応)を、完了した**
(PR #73、マージ済み)。プレイ中(入力欄がある間)は、モバイルの下部固定バーを隠す
(`document.body[data-hide-bottom-nav]`。`view.js`の`showView()`が制御)。ソフトウェアキーボードが
開いたとき(`window.visualViewport`の`resize`。物理キーボード・デスクトップでは、ほぼ発生しない)、
入力欄にフォーカスがある間だけ、見える位置へスクロール(`input`の`focus`イベント自体では動かさない)。
ゲームの`input.focus()`すべてに`{ preventScroll: true }`を追加(**実ブラウザのE2Eで発見した、この
PRとは無関係の既存のブラウザ既定動作**: デスクトップの、やや低いウィンドウ高さで、ゲーム開始のたびに
意図せず画面が動いていたのを修正)。横向き(landscape)の低い画面向けのCSS(`game.css`の末尾。**CSS
のカスケードの都合で、他の`.scene`等のルールより、あとに書く必要がある**。ヘッダーの高さ・`.main`の
余白・ゲージ/場面/単語カードの余白と文字サイズを圧縮。並び順は変えない)。kii-michiの「キーを押して
入力」は、物理キーボード前提(`event.code`)の既知の制限として、対応しないことを明記(ツールの目的
自体が物理キーボードを前提にしているため)。単体テスト1890件・実ブラウザE2E9件、すべて成功。
決定・テスト結果・実装中に見つけた3つの不具合とその修正の記録は docs/decisions/0047-phase-22-plan.md。
CLAUDE.md に「ソフトウェアキーボード・外付けキーボード・縦横自動調整(Phase 22)」節を追記。

**Phase 21(ライト・ダーク・システムテーマ・文字サイズ・アニメーション軽減・キーボード操作点検・
コントラスト点検・アカウント同期)は、この前にすべて完了した**(PR1=#67・PR2=#69・PR3=#71)。

Next Action:
Phase 22 PR2(タッチ操作の点検・改善)の詳細を、0047 決定ログに追記してから着手する。役職/職種の
ラジオボタン・チャートのホバー領域など、既存の`--tap-size`(44px)トークンの適用範囲を、実際に
タッチ操作(Puppeteerのタッチエミュレーション)で確認し、問題が見つかれば、この PR で直す想定。
