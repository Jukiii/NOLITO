...
補足。監査ログの保存期間・海外事業者の明記)を PR #51 で実施・マージ・本番確認した。
残り(独自ドメイン・お問い合わせフォームの有効化・利用規約)は、運営者の判断・作業待ち(#19 にコメント済み)。
...
**#19 の内訳(2026-09-24 におすすめをコメント)**:

- プライバシーポリシーの補足(監査ログ保存期間・海外事業者の明記)は、実施・マージ・本番確認済み(PR #51)
- 残り(いずれも運営者の判断・作業): 独自ドメインの取得(推奨: Google審査より先に)/ お問い合わせフォームを正式な連絡先にする(`docs/contact-setup.md` の手順。本番はまだ `CONTACT_ENABLED=false`)/ 利用規約の要否とタイミング / Google同意画面の「公開済み」化
