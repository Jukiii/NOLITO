...
| 最終更新日時   | 2026-09-24                                               |
| 現在のフェーズ | Phase 19(PR 1・PR 2 完了。本番マイグレーション適用待ち） |
| 現在の状態     | BLOCKED                                                  |
...
| Phase 19 | 未登録プレイ+アカウント移行・公開範囲設定・オンラインランキング          | **BLOCKED**   | PR1=#56(済), PR2=#58(済・本番マイグレーション適用待ち), PR3=未着手 | -                                              | PR2 の本番 D1 マイグレーション(0004)適用が、運営者の対応待ち(0043決定ログ） |
...
```text
Phase 19(PR 1・PR 2 完了。PR 3: プロフィール公開範囲・オンラインランキング、は未着手)
```

## Current Status

```text
BLOCKED(コードは完了・マージ済み。本番 D1 へのマイグレーション適用だけが、ツールの制限で止まっている)
```

## Current Branch

```text
main(PR 2 は #58 でマージ済み。PR 3 用のブランチは、まだ作成していない)
```

## Current Task

```text
migrations/0004_game_sync.sql を、本番の D1 に適用する(node scripts/d1-remote.mjs execute nolito
--remote --file migrations/0004_game_sync.sql)。1回目は Cloudflare API 側の一時的な認証エラー
(Authentication error, code 10000)、2回目の再試行は、このセッションのサンドボックスの安全装置
(auto mode classifier)により、実行がブロックされた。DB は失敗時に元の状態のまま(未適用)。
運営者が、ローカルで上記コマンドを実行するか、再試行を明示的に許可することが必要。
```

## Current Step

```text
本番マイグレーション適用待ち(運営者の対応が必要)
```
...
```text
Phase 19 PR 1(記録のJSON書き出し・読み込み)を実装・テスト・マージ・本番確認まで完了した。
次は PR 2(アカウント移行)だが、着手前に、プライバシーポリシーの改定内容を運営者に確認する必要がある。
```

### 次に行う作業

```text
1. アカウント移行(ゲームの記録を、ログイン中のアカウントに保存する)の、プライバシーポリシーへの
   影響(新しく送るようになる情報・保存先・保存期間)を整理する
2. 運営者に、改定内容を確認してもらう(ポリシーの版を上げる、同意の取り直しが必要になる)
3. 確認が取れたら、phase-19-account-sync ブランチ(main から)で、PR 2 に着手する
```

### 最後に完了した作業

```text
Phase 19 PR 1: 記録の書き出し・読み込み(PR #56、マージ済み、本番 0.24.0 で確認済み)

Issue #19(アカウントの一般公開の前提)への提案をコメントし、実施できる範囲(プライバシーポリシーの
補足・利用規約の新設・お問い合わせフォームの本番有効化)を PR #51・#54 で実施・マージ・本番確認した。
独自ドメインは、運営者の指示により、一旦見送り(#19にコメント済み)。
```
...
```text
(PR #56 のマージコミット。git log --oneline -1 で確認)
```

## Pull Request

```text
直近マージ: #56(Phase 19 PR 1)、#55(chore)、#54(利用規約・お問い合わせ有効化)、#51(ポリシー補足）、
#50(chore)、#49(Phase 18 PR 4)、#48(fix: auth-flow.test.js)、#47(Phase 18 PR 3)
オープン中: なし
```
...
| Phase 19 PR2 | 2026-09-24     | #58           | ゲームの記録のアカウント移行(同期。本番マイグレーション適用待ち) |
...
Phase:
Phase 19 PR 3(プロフィール公開範囲の設定・オンラインランキング)。未着手

Status:
BLOCKED(PR2 のコードはマージ済み。本番 D1 へのマイグレーション適用だけが未完了)

Branch:
main(PR2 は #58 でマージ済み。PR3 用ブランチは、まだ未作成)

Task:
migrations/0004_game_sync.sql を、本番の D1 に適用する。適用できるまで、本番の「ゲームの記録の
アカウント保存」機能は、実際には保存できない(サーバー側の game_progress テーブルがないため)

Last Completed:
Phase 19 PR 2(ゲームの記録のアカウント移行・同期)。PR #58 マージ済み(コード・テスト・
プライバシーポリシー改定(版3)・ローカルでの実ブラウザ E2E(18件)まで完了)。決定は
docs/decisions/0043-phase-19-account-sync.md

Next Action:
1. 運営者が、`node scripts/d1-remote.mjs execute nolito --remote --file migrations/0004_game_sync.sql`
   を実行するか、この操作の再試行を Claude Code に明示的に許可する
2. 適用できたら、`curl https://nolito.pages.dev/api/games/escape-boss/sync`(ログイン状態で)などで、
   本番の動作を確認する
3. そのあと、Phase 19 PR 3(プロフィール公開範囲の設定・オンラインランキング)に着手する

Human Task:
migrations/0004_game_sync.sql の、本番 D1 への適用(上記。1回目は Cloudflare API 側の一時的な
認証エラー、2回目の再試行は、このセッションのサンドボックスの安全装置によりブロックされた。
DB は未適用のまま、安全な状態)。
ほかに、Issue #12・#13・#19 は WAITING_HUMAN だが、いまの作業の直接のブロッカーではない

Blocker:
本番 D1 へのマイグレーション適用(上記)
