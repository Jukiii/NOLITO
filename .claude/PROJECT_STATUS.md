...
| 最終更新日時   | 2026-09-26                                 |
| 現在のフェーズ | Phase 26(管理画面・RBAC)。着手前           |
| 現在の状態     | IN_PROGRESS                                |
...
```text
Phase 25(AI活用: 改善提案・類似語チェック・流れの文書化)完了。次は Phase 26(管理画面・RBAC)
```

## Current Status

```text
IN_PROGRESS(Phase 25 完了。Phase 26 は着手前)
```

## Current Branch

```text
main(Phase 25 PR3 は #91 でマージ済み。Phase 26 用のブランチは、まだ作成していない)
```

## Current Task

```text
Phase 26(管理画面・RBAC)の仕様確認(docs/01_phases/phase-26.md)・実装計画。
```

## Current Step

```text
docs/01_phases/phase-26.md を読み、既存のPhase 25 と同様、チャットで運営者に実装スコープを確認してから着手する
```
...
```text
Phase 25(AI活用: 改善提案・類似語チェック・流れの文書化)を実装・テスト・マージまで完了した。
次は Phase 26(管理画面・RBAC)。
```

### 次に行う作業

```text
1. docs/01_phases/phase-26.md(管理画面・RBAC)を確認する
2. 既存の関連方針(「管理画面・AIのAPIとの連携は作らない」= 0026決定ログ。有料のAIに依存しない方針)
   を踏まえ、実装方針・PR分割を決める。仕様の解釈に迷えば、Phase24・25と同様、チャットで確認する
3. 新しいブランチ(main から)で、Phase 26 に着手する
```

### 最後に完了した作業

```text
Phase 25: AI活用: 改善提案・類似語チェック・流れの文書化(PR1=#89・PR2=#90・PR3=#91、マージ済み)。
チャットで運営者に確認し、新しく作る3点(改善提案・類似語チェック・流れの文書化)の承認を得た
(生成・チェック・人間確認必須・有料APIに依存しないという仕様の大半は、すでにPhase11・14で実装済み
だった)。

**PR1(改善提案)**: vocab-stats.mjsにimprovementCandidates(公開済み・関連用語が空の語を選ぶ純粋
関数)を追加。vocab-check.mjsに--improveオプションを追加(docs/06_ai/content-improve-prompt.md
新規)。確認済みかどうかは問わない。実データで180語中68語が候補。

**PR2(類似語チェック)**: scripts/lib/vocab-similarity.mjs(新規)に、explanationの文字2-gramの
Dice係数で、しきい値(既定0.6)以上の組を見つけるsimilarPairsを追加(有料APIは使わない)。vocab-
check.mjsの警告(プレーン・--for-aiの両方)に追加。実データでしきい値0.6で2組(1件は実際に意味が
近い指摘「仕込み/下ごしらえ」、1件は言い回しが近いだけの誤検出「顧客/納期」)。

**PR3(流れの文書化)**: docs/06_ai/README.md(新規)に、生成→機械チェック→AIチェック→人間の
最終確認→確認済みにする→公開→(公開後)改善提案、という全体の流れをまとめた。有料APIとの自動
連携は作らない・管理画面アップロードはPhase26待ちを明記。

単体テスト1997件、すべて成功。決定・テスト結果はdocs/decisions/0050-phase-25-plan.md。CLAUDE.md
に3つの節を追記。

**Phase 24(語録の拡張ファイル・専用語)は、この前に完了した(PR #87)**。
```
...
```text
直近マージ: #91(Phase 25 PR3。Phase 25 完了)、#90(Phase 25 PR2)、#89(Phase 25 PR1)、
...
Phase:
Phase 26(管理画面・RBAC)。**Phase 25(AI活用: 改善提案・類似語チェック・流れの文書化)は完了**

Status:
IN_PROGRESS

Branch:
main(Phase 25 PR3 は #91 でマージ済み。Phase 26 用ブランチは、まだ未作成)

Task:
Phase 26(管理画面・RBAC)の仕様確認(`docs/01_phases/phase-26.md`)・実装計画

Last Completed:
**Phase 25(AI活用: 改善提案・類似語チェック・流れの文書化)を、完了した**(PR1=#89・PR2=#90・
PR3=#91、マージ済み)。仕様の大半(生成・チェック・人間確認必須・有料APIに依存しない)は、すでに
Phase11・14で実装済みだったため、チャットで運営者に確認し、新しく作る3点の承認を得た(**Issue を
作らず、最初からチャットのAskUserQuestionで確認 → 即座に回答**。Phase24に続き2回目の成功で、
この運用を標準の手順にすることを、13章に記録した)。

**PR1(改善提案)**: `vocab-stats.mjs`に`improvementCandidates`(公開済み・関連用語が空の語を選ぶ
純粋関数)。`vocab-check.mjs`に`--improve`オプション(`docs/06_ai/content-improve-prompt.md`新規)。
確認済みかどうかは問わない。実データで180語中68語が候補。

**PR2(類似語チェック)**: `scripts/lib/vocab-similarity.mjs`(新規)に、explanationの文字2-gramの
Dice係数で、しきい値(既定0.6)以上の組を見つける`similarPairs`(有料APIは使わない)。実データで
しきい値0.6で2組(1件は実際に意味が近い指摘「仕込み/下ごしらえ」、1件は言い回しが近いだけの誤検出
「顧客/納期」。「しきい値以上でも必ず重複とは限らない」という位置づけを、そのまま裏づけた)。

**PR3(流れの文書化)**: `docs/06_ai/README.md`(新規)に、生成→機械チェック→AIチェック→人間の
最終確認→確認済みにする→公開→(公開後)改善提案、という全体の流れをまとめた。有料APIとの自動
連携は作らない・管理画面アップロードはPhase26待ちを明記。

単体テスト1997件、すべて成功。決定・テスト結果は`docs/decisions/0050-phase-25-plan.md`。CLAUDE.md
に3つの節を追記。

**Phase 24(語録の拡張ファイル・専用語のしくみ)は、この前に完了した(PR #87)**。

Next Action:
Phase 26(管理画面・RBAC)に着手する。まず`docs/01_phases/phase-26.md`を読み、既存方針(0026決定
ログ「管理画面・AIのAPIとの連携は作らない」・有料のAIに依存しない方針)との整合を確認する。仕様の
解釈に迷う点があれば、**Issueを作る前に、まずチャットのAskUserQuestionで確認する**(Phase24・25で
2回連続、この順序のほうが速く回答を得られたため。標準の手順)。
